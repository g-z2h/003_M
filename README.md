...
- Notion にサムネ画像保存
  [Notion ページ](https://nasal-howler-18d.notion.site/229a3d3196454d45b1dc97bb7de26626?v=c9a8417d963245cbbe1bf0eeab3ec240)

- 友達追加された人全員にメッセージ送る → ブロードキャストで対応（友達全員に送信可能）
- YouTube 動画複数取得 (複数チャンネル ID で絞るは可能？クエリで'肩トレ'ように絞る。複数クエリで絞れるか？)
  → チャンネルごとに API キー発行。クエリには複数クリエ（肩トレ、腕トレなど）を指定

- LINE Flex Message で~~取得した数だけ~~カルーセル表示 → 現段階では、動画取得時に投稿日の指定をしていないため、チャンネルの中のクエリにひっかかる最新動画を取得できるようになっている。（最新動画が更新されない限り同じ動画が届く。）理由としては、カルーセルを 5 つ分用意していても、動画の取得数が 4 つだとエラーになるから。 カルーセルでの表示をあきらめ、1 個ずつ送る最新動画を送る方法しか解決策はないのか？
